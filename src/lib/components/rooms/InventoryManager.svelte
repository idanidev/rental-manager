<script>
  ...
  function addCommonItem(itemName) {
    const exists = inventory.find(item => item.name.toLowerCase() === itemName.toLowerCase());
    if (!exists) {
      addItem(itemName);
    }
  }
...
<div class="space-y-4">
  ...
  <div class="bg-blue-50 border border-blue-200 rounded-xl p-4">
    <p class="text-sm font-medium text-blue-800 mb-2">Items comunes (click para añadir):</p>
    <div class="flex flex-wrap gap-2">
      {#each commonItems as item}
        <button
          type="button"
          on:click={() => addCommonItem(item)}
          class="px-3 py-1 bg-white border border-blue-300 text-blue-700 text-xs rounded-lg hover:bg-blue-100 transition-colors"
        >
          + {item}
        </button>
      {/each}
  ...
  {:else}
    <div class="text-center py-8 glass-card">
      ...
      <p class="text-gray-600 text-sm">
        No hay items en el inventario. Añade items para documentar lo que se entrega con la habitación.
      </p>
    </div>
  {/if}
