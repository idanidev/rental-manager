  ...
  async function loadUserData() {
    ...
    try {
      ...
      const processedInvitations =
        await permissionsService.processPendingInvitations(
          $userStore.id,
          $userStore.email,
        );
      ...
      if (processedInvitations.length > 0) {
        const propertyNames = processedInvitations
          .map((p) => p.name)
          .join(", ");
        showToast(
          `¡Bienvenido! Se te ha dado acceso a: ${propertyNames}`,
          "success",
        );
      }
  ...
  onMount(async () => {
    ...
    try {
      ...
      if ($userStore && $page.url.pathname === "/login") {
        ...
        !$page.url.pathname.startsWith("/accept-invitation")
      ) {
        goto("/login");
      }
    } catch (err) {
...
{:else}
  <div class="min-h-screen">
    {#if $userStore && !isAuthPage}
      <!-- Navbar Simple -->
      <nav
        class="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-3xl m-2 sm:m-4 mb-2 p-2 sm:p-3 relative z-50 shadow-xl"
      >
        <div class="flex items-center justify-between gap-4">
          ...
          >
            ...
            <span class="text-lg sm:text-xl font-bold gradient-text"
              >Rental Manager</span
            >
          </a>
          ...
          <div class="flex items-center gap-3">
            ...
            >
              ...
              >
                <div
                  class="w-8 h-8 rounded-full bg-gradient-to-br from-orange-500 to-amber-500 flex items-center justify-center text-white font-semibold text-sm flex-shrink-0"
                >
                  {$userStore?.user_metadata?.name?.[0]?.toUpperCase() ||
                    $userStore?.email?.[0]?.toUpperCase() ||
                    "U"}
                </div>
                <Menu
                  size={16}
                  class="text-gray-600 dark:text-gray-400 transition-transform {userMenuOpen
                    ? 'rotate-90'
                    : ''}"
                />
              </button>
                ...
                >
                  ...
                  >
                    {#if $theme === "dark"}
                      <Sun
                        size={16}
                        style="color: rgb(243, 244, 246) !important;"
                      />
                      <span>Modo claro</span>
                    {:else}
                      <Moon
                        size={16}
                        style="color: rgb(17, 24, 39) !important;"
                      />
                  ...
                  <button
                    ...
                    style="position: relative; z-index: 999999; color: {$theme ===
                    'dark'
                      ? 'rgb(248, 113, 113)'
    ...
    <!-- Main Content -->
    <main class="p-2 sm:p-4 pb-24 lg:pb-4">
      <slot />
    </main>
